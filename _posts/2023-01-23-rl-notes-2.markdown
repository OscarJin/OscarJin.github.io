...
$Q_t(a) \dot{=} \frac{\mathrm{sum~of~rewards~when}~a~\mathrm{taken~prior~to~}t}{\mathrm{number~of~times~}a~\mathrm{taken~prior~to~}t} = \frac{\sum_{i=1}^{t-1} R_i \cdot \mathbb{1}_{Ai=a} } {\sum_{i=1}^{t-1} \mathbb{1}_{Ai=a}}$
